...
def create_app() -> FastAPI:
    ...
    # Allow the Cloudflare-hosted UI (and any localhost variant) to fetch
    # this backend cross-origin. Browsers treat 127.0.0.1 as a "secure
    # context", so HTTPS pages can talk to it as long as CORS is open.
    cors_origins = [
        "http://127.0.0.1:8000",
        "http://localhost:8000",
        "https://epub-chinese-convert.pages.dev",
    ]
    extra = os.environ.get("EPUBCONV_CORS_ORIGINS", "")
    if extra:
        cors_origins.extend(o.strip() for o in extra.split(",") if o.strip())
    app.add_middleware(
        CORSMiddleware,
        allow_origins=cors_origins,
        allow_origin_regex=r"https://.*\.pages\.dev$",  # preview deploys
        allow_methods=["*"],
        allow_headers=["*"],
    ...
    # Fall back to the inline INDEX_HTML if pages/app.html is missing
    # (e.g. running from a wheel where pages/ wasn't packaged).
    app_html_path = repo_root / "pages" / "app.html"
    ...
    def index() -> str:
        ...
        if app_html_path.exists():
            ...
            html = INDEX_HTML
        return html.replace("__ENGINE_OPTIONS__", opts)
